...
int main() {
  ...
  // 16. 字典序单项式序满足乘法相容性 —— 这是多项式长除法必然终止的前提
  {
    ...
    CHECK_TRUE(detail::compareLex(xv, yv) == std::strong_ordering::greater);
    // 两边同乘 x，顺序保持：x > y ⟹ x^2 > x*y
    CHECK_TRUE(detail::compareLex(x2, xy) == std::strong_ordering::greater);
    // 两边同乘 y，顺序保持：x > y ⟹ x*y > y^2
    CHECK_TRUE(detail::compareLex(xy, y2) == std::strong_ordering::greater);

    // 同变量比指数；缺失的变量按指数 0 处理
    CHECK_TRUE(detail::compareLex(x2, xv) == std::strong_ordering::greater);
    CHECK_TRUE(detail::compareLex(xv, x2) == std::strong_ordering::less);
    CHECK_TRUE(detail::compareLex(xv, xv) == std::strong_ordering::equal);
    // 常数项最小
    CHECK_TRUE(detail::compareLex(xv, VarPowers{}) == std::strong_ordering::greater);
  }
