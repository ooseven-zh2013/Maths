...
- 需要「首项」时必须用 `maths_detail::compareLex`（字典序），不能用 `terms.begin()`
...
| 序 | 函数 | 用途 |
| --- | --- | --- |
| 字典序（lex） | `maths_detail::compareLex` | 数学运算：取首项、长除法 —— **必须用它** |
| 展示序 | `maths_detail::displayOrderLess` | 仅用于 `str()` / `latex()` 的打印顺序 |
