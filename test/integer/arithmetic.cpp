...
int main() {
  ...
  // 5. 幂运算（正指数）
  std::cout << "5. 幂运算测试（正指数）:" << '\n';
  Integer base1(2LL);
  Integer exp1(10LL);
  Fraction result1 = base1.pow(exp1);
  std::cout << base1 << "^" << exp1 << " = " << result1 << " (应该是 1024)" << '\n';

  Integer base2(-3LL);
  Integer exp2(3LL);
  Fraction result2 = base2.pow(exp2);
  std::cout << base2 << "^" << exp2 << " = " << result2 << " (应该是 -27)" << '\n';

  Integer base3(-2LL);
  Integer exp3(4LL);
  Fraction result3 = base3.pow(exp3);
  std::cout << base3 << "^" << exp3 << " = " << result3 << " (应该是 16)" << '\n' << '\n';

  // 6. 幂运算（负指数）
  std::cout << "6. 幂运算测试（负指数）:" << '\n';
  Integer base4(2LL);
  Integer exp4(-3LL);
  Fraction result4 = base4.pow(exp4);
  std::cout << base4 << "^" << exp4 << " = " << result4 << " (应该是 1/8)" << '\n';

  Integer base5(-2LL);
  Integer exp5(-3LL);
  Fraction result5 = base5.pow(exp5);
  std::cout << base5 << "^" << exp5 << " = " << result5 << " (应该是 -1/8)" << '\n';

  Integer base6(-3LL);
  Integer exp6(-2LL);
  Fraction result6 = base6.pow(exp6);
  std::cout << base6 << "^" << exp6 << " = " << result6 << " (应该是 1/9)" << '\n' << '\n';

  // 7. 幂运算（零指数）
  std::cout << "7. 幂运算测试（零指数）:" << '\n';
  Integer base7(123LL);
  Integer exp7(0LL);
  Fraction result7 = base7.pow(exp7);
  std::cout << base7 << "^" << exp7 << " = " << result7 << " (应该是 1)" << '\n' << '\n';
